	...
	var hz6d_from_page = hz6d_getCookie("53kf_70697378_keyword");
	var kf_70697378_keyword_ok = hz6d_getCookie("kf_70697378_keyword_ok");
	if (kf_70697378_keyword_ok != 1)
	{
		hz6d_from_page = document.referrer;
	}
	document.cookie = "53kf_70697378_keyword=" + hz6d_from_page;
	document.cookie = "kf_70697378_keyword_ok=1";
	...
	function hasdoctype()
  {
	...
	function setIsinvited()
	{
		try
		{
			...
			if (acc_autotype == 3)
			{
				document.cookie = "onliner_zdfq70697378=" + onliner_zdfq;
			}
		}
		catch (e)
		{}
	}
	var onliner_zdfq = hz6d_getCookie("onliner_zdfq70697378"); // onliner_zdfq: 0.初始值 2.点击接受 3.点击拒绝 
	if (onliner_zdfq == "")
	{
		onliner_zdfq = 0;
		document.cookie = "onliner_zdfq70697378=" + onliner_zdfq;
	...
	// 加载ivt.php，即中间的接受邀请层 
	if (!hz6d$("ivt_script"))
		document.write("<scr"+"ipt src='http://www3.53kf.com/kf_ivt.php?arg=xinjingying&test_do=&style=1&isonline=1&kfonline=1&lang=zh-cn&resize=yes&charset=utf-8&kflist=off&kf=&zdkf_type=1"+ hz6d_referer + hz6d_from_page + "&lytype=0&tpl_name=crystal_blue&tpl_width=703&tpl_height=473' type='text/javascript' id='ivt_script'></scr"+"ipt>");  
	...
	if (kf_script_num <= 1)
  {
      ...
      for (var i = 0; i < element_arrs.length; i++)
      {
        ...
        else
        {
					switch (tmp_e_type)
          {
            ...
            case 'etel': click_str = 'window.open(\'http://chat.53kf.com/eht.php?company_id=70697378&style_id=103645046\',\'_blank\',\'height=400,width=300,top=100,left=200,status=yes,toolbar=no,menubar=no,resizable=no,scrollbars=no,location=no,titlebar=no\')'; break;
            case 'qq':
	...
	if (!hz6d$("stat_script"))
		document.write("<scr"+"ipt src='http://www3.53kf.com/stat.php?com_id=70697378" + hz6d_referer + hz6d_from_page + "' type='text/javascript' id='stat_script'></scr"+"ipt>");